...
import { FaMoneyBillTransfer, FaRegEye, FaRegPenToSquare, FaUserPlus } from 'react-icons/fa6'
...
const Procurement = () => {
    ...
    const { data: payments, isLoading: paymentLoading, isError } = useGetAllPayments()


    const actionTemplate = (rowData: any) => {
        return (
            <div className="flex flex-row gap-[20px] items-center">
                <div onClick={() => { setViewedData(rowData); setIsView(rowData) }} className="cursor-pointer">
                    <span className=' cursor-pointer px-2 border p-2 rounded-[4px] hover:bg-gray-100'><FaRegEye size={20} /></span>
                </div>
            </div>
        )
    }
        ...
        <>
            <div className='flex flex-col gap-[20px] p-4'>
                ...
                <div className='grid grid-cols-2 gap-[20px]'>
                    <Cards value={payments?.filter((item: any) => item.payeeType == "Supplier").length} icon={<FaMoneyBillTransfer color='white' size={20} />} title={"Total Payments"} color={"bg-blue-100"} />
                    <Cards value={suppliers?.supplier.length} icon={<FaUserPlus color='white' size={20} />} title={"Total Supplier"} color={"bg-blue-100"} />
                </div>
                <div className='w-full bg-white rounded-[6px] p-4 flex flex-row items-center justify-between'>
                    <div className='flex flex-row gap-[10px] items-center'>
                        <span className='text-[14px] font-[600]'>Latest Payments</span>
                        <div className='px-2 bg-gray-100 py-[2px] text-[14px] font-[600]'>{!isLoading ? payments?.filter((item: any) => item.payeeType == "Supplier").length : 0}</div>
                    </div>
