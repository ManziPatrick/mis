...
import { useGetAllReceivedNotes, useCreateReceivedNote, useApproveReceivedNote, useDeleteReceivedNote, useUploadFormProof } from '@/utlis/hooks/forms.hook'
...
const emptyItem = { item: '', specifications: '', unit: 'pcs', quantity: 1, remarks: '' }

const ReceivedNotes = () => {
  const { data, isLoading, refetch } = useGetAllReceivedNotes()
  const { mutate: create, isPending: creating } = useCreateReceivedNote()
  const { mutate: approve } = useApproveReceivedNote()
  const { mutate: del } = useDeleteReceivedNote()
  const { mutate: uploadProof } = useUploadFormProof('received-notes')
  ...
  const [committee, setCommittee] = useState([{ name: '' }, { name: '' }])
  ...
  const filtered = useMemo(() => {
    if (!data) return []
    return data.filter((o: any) => o.grnNumber?.toLowerCase().includes(search.toLowerCase()) || o.supplierName?.toLowerCase().includes(search.toLowerCase()))
  }, [data, search])

  const updateItem = (idx: number, field: string, value: any) => setItems(items.map((item, i) => i === idx ? { ...item, [field]: value } : item))

  const handleCreate = () => {
    if (!supplierName || !tradeNumber || !date) { toast.error('Fill all required fields'); return }
    create({ poNumber, tradeNumber, date, supplierName, items, committeeMembers: committee.filter(d => d.name) }, {
      onSuccess: () => { toast.success('Goods Received Note created'); setIsOpen(false); refetch() },
  ...
  const actionTemplate = (row: any) => (
    <div className='flex gap-2'>
      <Link href={`/stock/forms/print/received-note/${row._id}`} target='_blank'><button className='p-2 rounded bg-blue-50 text-blue-700'><FiPrinter size={14} /></button></Link>
      {row.status === 'pending' && <>
  ...
  return (
    <div className='flex flex-col gap-4 p-6'>
      <div className='flex justify-between items-center'>
        <div><p className='text-[13px] text-gray-500'>Stock / Forms /</p><h1 className='text-[20px] font-[700]'>Annex III: Goods Received Notes</h1></div>
        <button onClick={() => setIsOpen(true)} className='flex items-center gap-2 bg-[#2E3487] text-white px-4 py-2 rounded-[8px] text-[13px] hover:opacity-80'><FiPlus size={16} /> New GRN</button>
      </div>
      <div className='flex justify-between items-center bg-white p-4 rounded-[8px]'>
        <span className='font-[600]'>All GRNs ({filtered.length})</span>
        <div className='flex items-center gap-2 border rounded-[6px] p-2 w-[280px]'><CiSearch size={18} /><input value={search} onChange={e => setSearch(e.target.value)} placeholder='Search by GRN No / Supplier' className='text-[13px] outline-none w-full' /></div>
      </div>
      {isLoading ? <div className='flex justify-center p-20'><OrbitProgress color="#2E3487" size="small" text="" textColor="" /></div>
        : !filtered.length ? <div className='flex flex-col items-center justify-center p-16 bg-white rounded-[8px]'><GiEmptyWoodBucket size={60} color='lightgray' /><span className='font-[700] mt-2'>No goods received notes found</span></div>
        : <DataTable value={filtered} paginator rows={10} className='w-full'>
          ...
          <Column field='grnNumber' header='GRN Number' headerClassName='bg-[#2E3487] text-white h-[50px] px-3 text-[13px]' bodyClassName='px-3 py-2 text-[13px] border-b font-[600]' />
      ...
      <Dialog header="New Goods Received Note" visible={isOpen} onHide={() => setIsOpen(false)} className='w-[90vw] max-w-[860px]'>
        <div className='flex flex-col gap-4 p-2'>
          <div className='grid grid-cols-2 gap-4'>
            ...
            <div className='flex flex-col gap-1'><label className='text-[12px] font-[600]'>GRN No. / PO Reference</label><input value={poNumber} onChange={e => setPoNumber(e.target.value)} className='border rounded-[6px] p-2 text-[13px] outline-none' placeholder='PO-XXXX' /></div>
            <div className='flex flex-col gap-1'><label className='text-[12px] font-[600]'>Supplier Name *</label><input value={supplierName} onChange={e => setSupplierName(e.target.value)} className='border rounded-[6px] p-2 text-[13px] outline-none' /></div>
          </div>
          <div className='border rounded-[8px] overflow-hidden'>
            <table className='w-full text-[12px]'>
              <thead><tr className='bg-[#2E3487] text-white'><th className='p-2 w-8'>#</th><th className='p-2 text-left'>Item</th><th className='p-2 text-left'>Specifications</th><th className='p-2 w-16'>Unit</th><th className='p-2 w-20'>Qty</th><th className='p-2 text-left'>Remarks</th><th className='p-2 w-8'></th></tr></thead>
              <tbody>
                {items.map((item, idx) => (
                  <tr key={idx} className='border-b'>
                    ...
                    <td className='p-1'><input value={item.remarks} onChange={e => updateItem(idx, 'remarks', e.target.value)} className='w-full border rounded p-1 text-[12px] outline-none' /></td>
          ...
          <div><p className='text-[12px] font-[700] mb-2'>Receiving Committee</p><div className='grid grid-cols-2 gap-2'>{committee.map((d, i) => <input key={i} value={d.name} onChange={e => setCommittee(committee.map((c, j) => j === i ? { name: e.target.value } : c))} className='border rounded p-2 text-[12px] outline-none' placeholder={`Member ${i + 1}`} />)}</div></div>
          <div className='flex gap-3 justify-end'>
            <button onClick={() => setIsOpen(false)} className='px-4 py-2 border rounded-[6px] text-[13px]'>Cancel</button>
            <button onClick={handleCreate} disabled={creating} className='px-4 py-2 bg-[#2E3487] text-white rounded-[6px] text-[13px] hover:opacity-80 disabled:opacity-50'>{creating ? 'Creating...' : 'Create GRN'}</button>
          </div>
        </div>
      </Dialog>
      <SignatureModal 
        ...
        onConfirm={(sig) => {
          const roleName = session?.user?.role === 'logistics' ? 'Logistics Officer' : 'Admin';
