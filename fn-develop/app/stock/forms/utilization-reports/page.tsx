...
import { useGetAllUtilizationReports, useCreateUtilizationReport, useApproveUtilizationReport, useDeleteUtilizationReport, useUploadFormProof } from '@/utlis/hooks/forms.hook'
...
const emptyItem = { item: '', quantityReceived: 0, totalStudents: 0, quantityUtilized: 0, balance: 0 }

const UtilizationReports = () => {
  const { data, isLoading, refetch } = useGetAllUtilizationReports()
  const { mutate: create, isPending: creating } = useCreateUtilizationReport()
  const { mutate: approve } = useApproveUtilizationReport()
  const { mutate: del } = useDeleteUtilizationReport()
  const { mutate: uploadProof } = useUploadFormProof('utilization-reports')

  const [isOpen, setIsOpen] = useState(false)
  const [search, setSearch] = useState('')
  const [tradeNumber, setTradeNumber] = useState('')
  const [rqfLevel, setRqfLevel] = useState('')
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((o: any) =>
      ...
      o.titleOfModule?.toLowerCase().includes(search.toLowerCase())
    )
  }, [data, search])
  ...
  const handleCreate = () => {
    if (!tradeNumber || !titleOfModule || !schoolYear) { toast.error('Fill required fields'); return }
    create({ tradeNumber, rqfLevel, titleOfModule, schoolYear, dateOfSubmission, items, preparedBy, checkedBy }, {
      onSuccess: () => { toast.success('Utilization Report created'); setIsOpen(false); refetch() },
  ...
  const actionTemplate = (row: any) => (
    <div className='flex gap-2'>
      <Link href={`/stock/forms/print/utilization-report/${row._id}`} target='_blank'><button className='p-2 rounded bg-blue-50 text-blue-700'><FiPrinter size={14} /></button></Link>
      {row.status === 'pending' && <>
  ...
  return (
    <div className='flex flex-col gap-4 p-6'>
      <div className='flex justify-between items-center'>
        <div><p className='text-[13px] text-gray-500'>Stock / Forms /</p><h1 className='text-[20px] font-[700]'>Annex VIII: Utilization Reports</h1></div>
        <button onClick={() => setIsOpen(true)} className='flex items-center gap-2 bg-[#2E3487] text-white px-4 py-2 rounded-[8px] text-[13px] hover:opacity-80'><FiPlus size={16} /> New Report</button>
      </div>

      <div className='flex justify-between items-center bg-white p-4 rounded-[8px]'>
        <span className='font-[600]'>All Utilization Reports ({filtered.length})</span>
        <div className='flex items-center gap-2 border rounded-[6px] p-2 w-[280px]'><CiSearch size={18} /><input value={search} onChange={e => setSearch(e.target.value)} placeholder='Search module / trade' className='text-[13px] outline-none w-full' /></div>
      </div>

      {isLoading ? <div className='flex justify-center p-20'><OrbitProgress color="#2E3487" size="small" text="" textColor="" /></div>
        : !filtered.length ? <div className='flex flex-col items-center justify-center p-16 bg-white rounded-[8px]'><GiEmptyWoodBucket size={60} color='lightgray' /><span className='font-[700] mt-2'>No reports found</span></div>
        : <DataTable value={filtered} paginator rows={10} className='w-full'>
          <Column field='tradeNumber' header='Trade' headerClassName='bg-[#2E3487] text-white h-[50px] px-3 text-[13px]' bodyClassName='px-3 py-2 text-[13px] border-b font-[600]' />
          <Column field='titleOfModule' header='Module' headerClassName='bg-[#2E3487] text-white h-[50px] px-3 text-[13px]' bodyClassName='px-3 py-2 text-[13px] border-b' />
      ...
      <Dialog header="New Utilization Report" visible={isOpen} onHide={() => setIsOpen(false)} className='w-[90vw] max-w-[850px]'>
        <div className='flex flex-col gap-4 p-2'>
          <div className='grid grid-cols-2 gap-4'>
            ...
            <div className='flex flex-col gap-1'><label className='text-[12px] font-[600]'>RQF Level *</label><input value={rqfLevel} onChange={e => setRqfLevel(e.target.value)} className='border rounded-[6px] p-2 text-[13px] outline-none' /></div>
            <div className='flex flex-col gap-1'><label className='text-[12px] font-[600]'>Title of Module *</label><input value={titleOfModule} onChange={e => setTitleOfModule(e.target.value)} className='border rounded-[6px] p-2 text-[13px] outline-none' /></div>
            ...
            <div className='flex flex-col gap-1'><label className='text-[12px] font-[600]'>Date of Submission *</label><input type='date' value={dateOfSubmission} onChange={e => setDateOfSubmission(e.target.value)} className='border rounded-[6px] p-2 text-[13px] outline-none' /></div>
          </div>
          ...
          <div className='border rounded-[8px] overflow-hidden'>
            <table className='w-full text-[12px]'>
              <thead><tr className='bg-[#2E3487] text-white'>
                ...
                <th className='p-2 text-left w-20'>Qty Rec.</th>
                <th className='p-2 text-left w-20'>Students</th>
                <th className='p-2 text-left w-20'>Utilized</th>
                <th className='p-2 text-left w-20'>Balance</th>
              ...
              <tbody>
                {items.map((item, idx) => (
                  <tr key={idx} className='border-b'>
                    ...
                    <td className='p-1'><input type='number' value={item.totalStudents} onChange={e => updateItem(idx, 'totalStudents', e.target.value)} className='w-full border rounded p-1 text-[12px] outline-none' /></td>
                    <td className='p-1'><input type='number' value={item.quantityUtilized} onChange={e => updateItem(idx, 'quantityUtilized', e.target.value)} className='w-full border rounded p-1 text-[12px] outline-none' /></td>
                    <td className='p-1'><input type='number' value={item.balance} onChange={e => updateItem(idx, 'balance', e.target.value)} className='w-full border rounded p-1 text-[12px] outline-none' /></td>
          ...
          <div className='grid grid-cols-2 gap-4'>
            <div className='flex flex-col gap-1'><label className='text-[12px] font-[600]'>Prepared By (Teacher)</label><input value={preparedBy} onChange={e => setPreparedBy(e.target.value)} className='border rounded-[6px] p-2 text-[13px] outline-none' /></div>
            <div className='flex flex-col gap-1'><label className='text-[12px] font-[600]'>Checked By (Workshop Asst)</label><input value={checkedBy} onChange={e => setCheckedBy(e.target.value)} className='border rounded-[6px] p-2 text-[13px] outline-none' /></div>
          </div>
      ...
      <SignatureModal 
        ...
        onConfirm={(sig) => {
          const roleName = session?.user?.role === 'logistics' ? 'Head of Department' : 'Admin';
