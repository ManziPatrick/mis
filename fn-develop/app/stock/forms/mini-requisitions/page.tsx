...
import { useGetAllMiniRequisitions, useCreateMiniRequisition, useApproveMiniRequisition, useDeleteMiniRequisition, useUploadFormProof } from '@/utlis/hooks/forms.hook'
...
const emptyItem = { item: '', quantityRequisitioned: 0, quantityReceived: 0, observation: '' }

const MiniRequisitions = () => {
  const { data, isLoading, refetch } = useGetAllMiniRequisitions()
  const { mutate: create, isPending: creating } = useCreateMiniRequisition()
  const { mutate: approve } = useApproveMiniRequisition()
  const { mutate: del } = useDeleteMiniRequisition()
  const { mutate: uploadProof } = useUploadFormProof('mini-requisitions')
  ...
  const [verifiedByAssistant, setVerifiedByAssistant] = useState('')
  ...
  const handleCreate = () => {
    if (!tradeNumber || !date) { toast.error('Fill required fields'); return }
    create({ tradeNumber, date, items, requestedBy, verifiedByAssistant }, {
      onSuccess: () => { toast.success('Mini Requisition created'); setIsOpen(false); refetch() },
  ...
  const actionTemplate = (row: any) => (
    <div className='flex gap-2'>
      <Link href={`/stock/forms/print/mini-requisition/${row._id}`} target='_blank'><button className='p-2 rounded bg-blue-50 text-blue-700'><FiPrinter size={14} /></button></Link>
      {row.status === 'pending' && <>
  ...
  return (
    <div className='flex flex-col gap-4 p-6'>
      <div className='flex justify-between items-center'>
        <div><p className='text-[13px] text-gray-500'>Stock / Forms /</p><h1 className='text-[20px] font-[700]'>Annex VI: Mini Requisitions</h1></div>
        <button onClick={() => setIsOpen(true)} className='flex items-center gap-2 bg-[#2E3487] text-white px-4 py-2 rounded-[8px] text-[13px] hover:opacity-80'><FiPlus size={16} /> New Mini Requisition</button>
      </div>

      <div className='flex justify-between items-center bg-white p-4 rounded-[8px]'>
        <span className='font-[600]'>All Mini Requisitions ({filtered.length})</span>
        <div className='flex items-center gap-2 border rounded-[6px] p-2 w-[280px]'><CiSearch size={18} /><input value={search} onChange={e => setSearch(e.target.value)} placeholder='Search voucher / trade' className='text-[13px] outline-none w-full' /></div>
      </div>

      {isLoading ? <div className='flex justify-center p-20'><OrbitProgress color="#2E3487" size="small" text="" textColor="" /></div>
        : !filtered.length ? <div className='flex flex-col items-center justify-center p-16 bg-white rounded-[8px]'><GiEmptyWoodBucket size={60} color='lightgray' /><span className='font-[700] mt-2'>No mini requisitions found</span></div>
      ...
      <Dialog header="New Mini Requisition" visible={isOpen} onHide={() => setIsOpen(false)} className='w-[90vw] max-w-[800px]'>
        <div className='flex flex-col gap-4 p-2'>
          ...
          <div className='border rounded-[8px] overflow-hidden'>
            <table className='w-full text-[12px]'>
              <thead><tr className='bg-[#2E3487] text-white'>
                ...
                <th className='p-2 text-left w-24'>Qty Req.</th>
                <th className='p-2 text-left w-24'>Qty Rec.</th>
          ...
          <div className='grid grid-cols-2 gap-4'>
            <div className='flex flex-col gap-1'><label className='text-[12px] font-[600]'>Requested By (Trainer)</label><input value={requestedBy} onChange={e => setRequestedBy(e.target.value)} className='border rounded-[6px] p-2 text-[13px] outline-none' /></div>
            <div className='flex flex-col gap-1'><label className='text-[12px] font-[600]'>Verified By (Workshop Assistant)</label><input value={verifiedByAssistant} onChange={e => setVerifiedByAssistant(e.target.value)} className='border rounded-[6px] p-2 text-[13px] outline-none' /></div>
          </div>
      ...
      <SignatureModal 
        ...
        onConfirm={(sig) => {
          const roleName = session?.user?.role === 'dht' ? 'DHT' : 'Admin';
          approve({ id: sigModal.id, action: 'approved', approvedByDHT: roleName, signatureData: sig }, {
...
export default MiniRequisitions
