"use client";
import { WorkshopAssistantSidebar } from "@/components/sidebar/RoleSidebars";
...
export default function Layout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        else if (status === "authenticated" && session.user.role === "workshopassistant") {
            setIsLoading(false);
            setIsAllowed(true);
        } else router.push("/");
    ...
    if (isAllowed) return (
        <div className="w-full flex flex-row bg-gray-50">
            <div className="w-[20%] fixed"><WorkshopAssistantSidebar /></div>
